            ...
            <div onTouchStart={handleTouchStart}
                ...
                <div className="content-hero relative w-full h-screen">
                    ...
                    <div key={currentIndex} className="relative w-full h-screen px-6 md:px-12">
                        <div className="h-1/3 badge-hero flex items-center justify-between text-white">
                            <h3 className="bg-gray-700/70 px-3 py-1 rounded-full text-xs md:text-sm border border-white/10 backdrop-blur-sm animate-slide delay-100">{currentMovie?.media_type?.toUpperCase()}</h3>
                            <h3 className="bg-gray-800 px-3 py-1 rounded-full text-xs md:text-sm border border-white/10 backdrop-blur-sm animate-slide delay-100">TRENDING</h3>
                        </div>
                        <div className="main-content w-full md:w-2/3 h-2/3 flex flex-col justify-center">
                            <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold text-white animate-slide delay-200">{currentMovie?.title?.toUpperCase() || currentMovie?.name?.toUpperCase()}</h1>
                            <div className="rating text-sm sm:text-base flex gap-2 animate-slide delay-200">
                                ...
                                <p className="text-gray-300">
                                    {currentMovie?.release_date?.slice(0, 4) || currentMovie?.first_air_date?.slice(0, 4)}
                                </p>
                            </div>
                            ...
                            <p className="text-sm md:text-base text-white line-clamp-2 animate-slide delay-400 mb-2">{currentMovie?.overview}</p>
                            <button onClick={()=>(handleClick(currentMovie?.id, currentMovie?.media_type))} className="bg-blue-600 text-white text-sm md:text-base px-4 py-2 rounded-xl w-fit animate-slide delay-500 cursor-pointer">
                                View details
                            </button>
                        </div>
                    </div>
