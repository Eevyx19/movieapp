...
const CategoryCard = ({item, mediaType}) => {
    return (
        <Link className="h-auto text-white bg-gray-800 overflow-hidden rounded-lg" to={`/${mediaType}/${item.id}`}>
            <img className="w-full max-h-72" src={`${import.meta.env.VITE_API_IMAGE_BACKDROP}/${item?.poster_path}`} alt="" />
